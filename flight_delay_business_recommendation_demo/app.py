...
def flight_recommendations(
    ...
    base_dt = datetime.combine(flight_date, departure_time)
    labels = ["Best match", "Later option", "Earlier option"]
    offsets = [0, 55, -70]
    recs = []
    for label, offset, code in zip(labels, offsets, airline_candidates_for_route(origin, dest, airline_code)):
        option_dt = base_dt + timedelta(minutes=offset)
        duration_minutes = int(max(45, min(720, (distance_miles / 500.0) * 60 + 35)))
        landing_dt = option_dt + timedelta(minutes=duration_minutes)
        numeric_seed = sum(ord(char) for char in f"{origin}{dest}{code}") + abs(offset)
        flight_number = f"{code}{100 + numeric_seed % 8900}"
        recs.append(
            {
                "label": label,
                "airline_code": code,
                "airline": airline_name_from_code(code),
                "flight_number": flight_number,
                "time": option_dt.strftime("%H:%M"),
                "arrival_time": landing_dt.strftime("%H:%M"),
                "detail": "Estimated option based on route, selected time, and airline hub context.",
                "url": flight_search_url(origin, dest, flight_date),
                "booking_urls": flight_booking_urls(origin, dest, flight_date),
            }
        )
...
live_recommended_flights = fetch_route_flight_recommendations(origin, dest, selected_date, selected_time)
recommended_flights = live_recommended_flights or flight_recommendations(origin, dest, selected_date, selected_time, airline_code, distance_miles)
flight_recommendation_source = "Aviationstack route lookup" if live_recommended_flights else "Estimated route suggestions"
...
st.caption(f"{flight_recommendation_source}. Booking sites show the final live price, availability, baggage rules, and refund policy.")
    ...
    with flight_col:
        ...
        st.markdown(
            ...
            <div class="flight-card">
                <div class="tag">{escape(str(option.get('label', 'Recommended')))}</div>
                <div class="flight-number">{escape(str(option.get('flight_number', 'Search route')))}</div>
                <div class="airline">{escape(str(option.get('airline', 'Airline shown on booking site')))}</div>
                <div class="time-row">
                    <div><span>Depart</span><strong>{escape(str(option.get('time', 'Check site')))}</strong></div>
                    <div><span>Arrive</span><strong>{escape(str(option.get('arrival_time', 'Check site')))}</strong></div>
                </div>
